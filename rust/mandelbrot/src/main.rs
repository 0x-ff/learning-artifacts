...
fn mandelbrot() {
    ...
    if args.len() != 5 {
        writeln!(std::io::stderr(), "Порядок вызова: mandelbrot FILES PIXELS UPPERLEFT LOWERRIGHT").unwrap();
        writeln!(std::io::stderr(), "Пример: {} mandel.png 111x750 -1.20,0.35 -1.0,0.20", args[0]).unwrap();
        std::process::exit(1);
    }
    ...
    let lower_right = parse_complex(&args[4]).expect("ошибка при разборе координат правого верхнего угла");
